...
export class AnnouncementsAdmin implements OnInit {
  private api = 'http://localhost/sia-api/dashboard.php';
  ...
  loadAnnouncements(): void {
    this.aLoading = true;
    this.http.get<any>(`${this.api}?action=get_announcements`, this.h()).subscribe({
  ...
  saveA(): void {
    ...
    this.http.post<any>(`${this.api}?action=${action}`, body, this.h()).subscribe({
      next: r => {
        this.aSaving = false;
        if (r.success) {
  ...
  deleteA(id: number): void {
    ...
    this.http.post<any>(`${this.api}?action=delete_announcement`, { id }, this.h()).subscribe({
  ...
  loadEvents(): void {
    this.eLoading = true;
    this.http.get<any>(`${this.api}?action=get_events`, this.h()).subscribe({
  ...
  deleteE(id: number): void {
    ...
    this.http.post<any>(`${this.api}?action=delete_event`, { id }, this.h()).subscribe({
